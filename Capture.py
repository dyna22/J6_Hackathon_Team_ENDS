...
def create_folder(folder_name):
    if not os.path.exists('./Dataset/words_captured/' + folder_name):
        os.mkdir('./Dataset/words_captured/' + folder_name)


def capture_images(gesName):
    create_folder(str(gesName))
    ...
    for loop in listImage:
        while True:
            ...
            if cv2.waitKey(1) == ord('c'):
                ...
                if t_counter <= 350:
                    img_name = "./Dataset/words_captured/" + str(gesName) + "/{}.png".format(training_set_image_name)
